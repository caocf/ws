...
<c:set var="inner_title" value="日报-定制会员" />
		...
		<div class="well" style="padding: 0px !important; padding-top: 19px !important;">
			<form id="queryForm" name="queryForm" class="form-horizontal" method="post">
				...
				<div class="control-group group-search">
					...
					<div class="controls controls-row">
						<button name="btnQuery" class="btn btn-primary" type="button" value="dzhy">
		...
		<div>
			...
			<c:if test="${not empty datas}">
				...
				<div style="text-align: right; margin: 5px 0px">
					<button name="btnExcel" class="btn" value="dzhy">导出到EXCEL文件</button>
				</div>
											...
											<c:when test="${itemStatus.index == 0}">
												<c:choose>
													<c:when test="${status.index == 1}">
														<td rowspan="9" class="tdTitle">商盟会员</td>
														<td colspan="2" class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 2||status.index == 3||status.index == 11||status.index == 12||status.index == 13}">
														<td colspan="2" class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 4}">
														<td rowspan="6" class="tdTitle">累计商盟会员</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 10}">
														<td rowspan="4" class="tdTitle">红钻会员</td>
														<td colspan="2" class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:otherwise>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
