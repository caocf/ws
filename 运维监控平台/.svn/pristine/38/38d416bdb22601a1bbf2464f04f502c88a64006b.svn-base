...
Validator.prototype = {
	...
	error : function(v,elm,useTitle) {
                ...
                return (elm && elm.title) ? (elm.title.charAt(0)=='!' ? elm.title.substring(1, elm.title.length) : (elm.title + error)) : error;
	...
	initialize : function(form, options){
					 ...
					 if(this.options.immediate) {
						 ...
						 for(var i = 0; i < elements.length; i++) {
                             ...
                             var bindMethod = (input.is(':radio') || input.is(':checkbox')) ? 'click' : 'input propertychange';
                             if (input.is('select')) bindMethod = 'change';
                             input.bind(bindMethod, function(ev) {
                                 // ie8 fix
                                 if (ev.originalEvent && ev.originalEvent.propertyName && ev.originalEvent.propertyName != 'value') return;
	...
	validate : function() {
				 ...
				 if(this.options.stopOnFirst) {
				   ...
				   if(!result && this.options.focusOnError) {
					   var first = $.grep(ValidationUtils.getFormElements(this.form),function(elm){return $(elm).hasClass('error')})[0];
					   if(first.select) first.select();
					   first.focus();
                       $("html,body").animate({scrollTop: $(first).offset().top - 80}, 100);
				   }
...
$.extend(Validation, {
	...
	newErrorMsgAdvice : function(name,elm,errorMsg) {
							var advice = '<label class="error" id="advice-' + name + '-' + ValidationUtils.getElmID(elm) +'" style="display:none">&nbsp;&nbsp;' + errorMsg + '</label>';
							var tagName = '';
						...
						switch (tagName) {
							...
							case 'radio':
								...
								break;
							default:
								if($(elm).attr('type')=='radio' && $(elm).parent().parent().parent().attr('class')=="radio inline") { 
							    	$(elm).parent().parent().parent().parent().children(':last').after(advice);
							    }else {
							    	elm.after(advice);
							    }
						}
...
Validation.addAllThese([
		...
		['validate-one-required', function (v,elm) {
			var p = elm.parent().parent().parent().parent().parent();
			var options = $(':input',p);
