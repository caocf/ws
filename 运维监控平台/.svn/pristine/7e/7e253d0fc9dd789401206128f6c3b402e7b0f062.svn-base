...
<c:set var="inner_title" value="日报-交易情况" />
		...
		<div class="well" style="padding: 0px !important; padding-top: 19px !important;">
			<form id="queryForm" name="queryForm" class="form-horizontal" method="post">
				...
				<div class="control-group group-search">
					...
					<div class="controls controls-row">
						<button name="btnQuery" class="btn btn-primary" type="button" value="jyqk">
		...
		<div>
			...
			<c:if test="${not empty datas}">
				...
				<div style="text-align: right; margin: 5px 0px">
					<button name="btnExcel" class="btn" value="jyqk">导出到EXCEL文件</button>
				</div>
											...
											<c:when test="${itemStatus.index == 0}">
												<c:choose>
													<c:when test="${status.index == 2}">
														<td class="tdTitle" rowspan="2">商品分类</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 4}">
														<td class="tdTitle" rowspan="5">渠道分类</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 9}">
														<td class="tdTitle" rowspan="5">支付方式</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 1 || status.index == 14 || status.index == 15 || status.index == 16 || status.index == 17 || status.index == 18 || status.index == 19 || status.index == 20}">
														<td class="tdTitle" colspan="2" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:otherwise>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:otherwise>
