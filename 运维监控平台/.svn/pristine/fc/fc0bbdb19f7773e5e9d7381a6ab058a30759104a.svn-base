...
<%--		<script type="text/javascript" src="${ctx }/static/ui/js/artDialog/plugins/iframeTools.js"></script>--%>
		...
		//查询操作
		function getQuery() {
			var task_name = $("#task_name").val().replace(/(^\s*)|(\s*$)/g,"");
			if(task_name==null||task_name==""){
				...
				if(isNaN(max_age)||isNaN(min_age)||isNaN(coin)||isNaN(score)){
					...
					if (coin == "" && score == "" && max_age == "" && min_age == "") {
						window.parent.location.href = "${ctx}/analysis/screening/insertData?flag=1&task_name="+ encodeURI(encodeURI(task_name))+"&brand=" + brand + "&sex=" + 
						sex +"&area_id="+area_id+"&scoreType="+scoreType+"&coinType="+coinType+"&coin="+coin+"&score="+score+"&min_age="+min_age+"&max_age="+max_age;
						//window.parent.location.reload();
					} else if ((min_age-0)<0|| (max_age-0)>100||(min_age-0)>100) {
						art.dialog({
							title:'',
							width:'150',
							height:100,
...
<%--			}--%>
		...
		function doOperator(id,name){
			...
			window.parent.document.getElementById('busiFrame').src = "${ctx}/analysis/screening/getTaskList?id="+id+"&type="+type+"&page=${page}";
					...
					<tr  id="${item[0] }">
						...
						<td class="center vcenter" id="show_${item[0] }">
							...
							<c:if test="${item[3]=='2'}">
								...
								<a href="javascript:operator('${item[0]}',4)",>修改名称</a>
								|
								<a target="_blank" href="<%=request.getContextPath() %>/downhdfs.zip?filePath=/user/hive/warehouse/t_user_sift/id=${item[0]}&symbol=3"> 导出</a>
								|
								<a href="javascript:operator('${item[0]}',2)">删除</a>
							</c:if>
						</td>
