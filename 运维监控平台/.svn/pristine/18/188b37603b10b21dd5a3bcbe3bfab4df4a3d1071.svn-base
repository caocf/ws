...
<c:set var="inner_title" value="订单概览" />
...
<script type="text/javascript">
	$(function() {
		...
		$('button[name=btnQuery]').click(function() {
			...
			frm.action = "${ctx}/order/statistics";
			frm.submit();
		});
		...
		$('button[name=btnExcel]').click(function() {
			var frm = document.getElementById("queryForm");
			frm.action = "${ctx}/order/statistics/toExcel";
			frm.submit();
		});
		...
		$("select[name='categoriesPidStr']").change(
				function() {
					$.ajax({
						...
						url : "${ctx}/order/statistics/category",
						dataType : "json",
			...
			<c:forEach var="item" items="${data}" varStatus="statusJ">
				<c:if test="${!statusJ.first}">
					<c:if test="${statusJ.index == 1}">
						<a name="btnChart" type="${statusJ.index}" class="btn btn-primary" style="width: 140px; margin-top: 10px; font-weight: bold">${item}</a>
					</c:if>
					<c:if test="${statusJ.index != 1}">
						<a name="btnChart" type="${statusJ.index}" class="btn" style="width: 140px; margin-top: 10px; font-weight: bold">${item}</a>
					</c:if>
				</c:if>
			</c:forEach>
		</c:if>
