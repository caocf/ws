...
function selectStoreShelf(selector,storeId,callback,opts){
	...
	var treeBox = {
		...
		$(selector).click(function(){
			...
			treeBox.setting.async.url = G_CTX_ROOT + '/item/select?shopId=' +  $("#"+storeId ).val();
...
function selectBrand(returnValueId,opts){
	...
	var url = G_CTX_ROOT + '/item/selectBrand';
...
function selectShop(storeId,selectedItems,callback,opts){
	...
	showDialog("选择门店", G_CTX_ROOT + "/shop/selectStoreShop?storeid="+storeId+"&ids=" + selectedItems ,function(innerDoc,innerWin){
		var ids = [];
		var txts = [];
