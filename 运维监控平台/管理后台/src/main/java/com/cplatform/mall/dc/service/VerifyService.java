...
import com.cplatform.mall.dc.model.VerifyInfo;
...
public class VerifyService {
	...
	public void init() throws SQLException {
		...
		String sql = "select * from t_verify_num where substr(collect_time,0,12)>=?";
		List<Map<String, String>> list = dbHelper.getMapList(sql,new Object[]{startTime});
		List<VerifyInfo> orderList = new ArrayList<VerifyInfo>();
		if(list!=null&&list.size()>0){
			for(Map<String,String> map:list){
				VerifyInfo order = new VerifyInfo();
				order.setId(map.get("seq_id"));
				order.setType(map.get("act_type"));
				order.setCollectTime(map.get("collect_time"));
				order.setNum(ChartDataUtil.doubleFormat(Double.parseDouble(map.get("verify_num")), "#.##"));
				orderList.add(order);
			}
		}
		// 列名称
		...
		if(orderList.size()>0){
			...
			for(int i=0;i<SEARCH_NUM;i++){
				...
				for(VerifyInfo o : orderList){
					if(o.getCollectTime().substring(0,12).equals(time)){
						if(o.getType().equals(Type.FZ.getType())){
							fz = o.getNum();
						}
						if(o.getType().equals(Type.LS.getType())){
							ls = o.getNum();
						}
					}
		...
		cache.put(Type.FZ, fzList);
		cache.put(Type.LS, lsList);
	}

	public enum Type { 
		FZ("founder"), LS("lashou"), FIELD("seriesList"), START("start"), END("end");
	...
	public Map<Type,List> getAllOrder(String start, String end, int type) throws SQLException {
		...
		if(type == 1){
			...
			sql = "select * from t_verify_num where substr(collect_time,0,12)>=? and substr(collect_time,0,12)<=?";
		}else if(type == 2){
			try {
				num = DateUtil.getDifTime(start, end)/(TIME_DIF*6)+1;
				sql = "select act_type,substr(collect_time,0,10) collect_time,sum(verify_num) verify_num " +
						"from t_verify_num " +
			...
			try {
				...
				sql = "select act_type,substr(collect_time,0,8) collect_time,sum(verify_num) verify_num " +
						"from t_verify_num " +
						"where substr(collect_time,0,12)>=? and substr(collect_time,0,12)<=?" +
			...
			try {
				...
				sql = "select act_type,substr(collect_time,0,6) collect_time,sum(verify_num) verify_num " +
						"from t_verify_num " +
						"where substr(collect_time,0,12)>=? and substr(collect_time,0,12)<=?" +
		...
		listMap.put(Type.FZ, fzList);
		listMap.put(Type.LS, lsList);
		return listMap;
	}
