...
<html>
	<head>
		...
		//查询操作
		function getQuery() {
			var shopName = parent.document.getElementById('shop_name').value;
			var shop_name = encodeURI(encodeURI(parent.document.getElementById('shop_name').value));
			var coin = $("#coin").val();
				...
				if(trim(shopName)==""){
					...
					if(shopName.length>40){
						art.dialog({
							...
							time:1,
							icon: 'warning',
							content:'商户名称不能超过40个字符'
						});
						return false;
					}else{
						window.location.href = "${ctx}/analysis/commercial/getItemList?flag=1&brand=" + brand + "&sex=" + sex +"&area_id="+area_id+"&scoreType="+scoreType+
						"&coinType="+coinType+"&coin="+coin+"&score="+score+"&min_age="+min_age+"&max_age="+max_age+"&shop_name="+shop_name+"&shopId=${shopId}";
		...
		function operator(id,type){
		  	if(type==1){
				art.dialog({
				    ...
				    ok: function () {
						$.ajax({ type:"POST",
						    url : '${ctx}/analysis/commercial/doOperator',
						    dataType : "json",
						    data:{
						    	id:id,
						    	type:1
		...
		function doOperator(id,name){
			...
			window.parent.document.getElementById('busiFrame').src = "${ctx}/analysis/commercial/getItemList?id="+id+"&type="+type+"&shop_name="+shop_name+"&page=${page}&shopId=${shopId}";
		}
		...
		function doBlur(obj,id){
			...
			//重置查询条件
		function doReset() {
			parent.document.getElementById('shop_name').value="";
			window.location.href = "${ctx}/analysis/commercial/getItemList";
		}
		//去掉字符串头尾空格   
		function trim(str) {   
				...
				<div class="tab-pane fade in active" id="home">
					...
					<div class="control-group group-search" style="margin-left: 0px;">
						<div class="controls controls-row">
							...
							<input type="text" id="score" name="score" maxlength="10" value="${score}"
								style="width: 100px; height: 40%;">
							</th>
							<th>
								商城币:
							...
							<input type="text" id="coin" name="coin" maxlength="10" value="${coin}"
								style="width: 100px; height: 40%;">
						</div>
						...
						<div class="controls controls-row">
							...
							<button class="btn btn-primary" type="button" onclick="doReset();">
					...
					<tr  id="${item[0] }">
						...
						<td class="center vcenter" id="show_${item[0] }">
							...
							<c:if test="${item[3]=='2'}">
								...
								<a href="javascript:operator('${item[0]}',4)",>修改名称</a>
								|
								<a target="_blank" href="<%=request.getContextPath() %>/downhdfs.zip?filePath=/user/hive/ExportUserTask/${item[0]}&symbol=1"> 导出</a>
								|
								<a href="javascript:operator('${item[0]}',2)">删除</a>
							</c:if>
						</td>
