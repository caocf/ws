		...
		<div class="well" style="padding: 0px !important; padding-top: 19px !important;">
			<form id="queryForm" name="queryForm" class="form-horizontal" method="post">
				...
				<div class="control-group group-search">
					...
					<div class="controls controls-row">
						<button name="btnQuery" class="btn btn-primary" type="button" value="dsywWeekly">
		...
		<div>
			...
			<c:if test="${not empty datas}">
				...
				<div style="text-align: right; margin: 5px 0px">
					<button name="btnExcel" class="btn" value="dsywWeekly">导出到EXCEL文件</button>
				</div>
											...
											<c:when test="${itemStatus.index == 0}">
												<c:choose>
													<c:when test="${status.index == 1}">
														<td class="tdTitle" rowspan="4">销售终端统计</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 5}">
														<td class="tdTitle" rowspan="4">代金券统计</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 9}">
														<td class="tdTitle" rowspan="4">电影票统计</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 13}">
														<td class="tdTitle" rowspan="4">汽车票统计</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 17}">
														<td class="tdTitle" rowspan="4">订餐统计</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:when test="${status.index == 21}">
														<td class="tdTitle" rowspan="4">门票统计</td>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
													</c:when>
													<c:otherwise>
														<td class="tdTitle" title="${hints[status.index]}">${item}</td>
