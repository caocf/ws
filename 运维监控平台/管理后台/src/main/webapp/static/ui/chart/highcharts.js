...
(function() {
	...
	function Gb(a, b, c) {
		...
		if (Ra) {
			...
			kc.appendChild(va.createTextNode(a + " {" + e + "}\n"))
	}
	function H(a, b) {
		a || (a = {});
		for ( var c in b)
	...
	function lc(a, b, c) {
		...
		if (!Qa(b))
			return "Invalid date";
		b = new Date(b * Sa);
		var e = b[mc](), f = b[nc](), g = b[Ib](), i = b[Rb](), k = b[Sb](), j = Ba.lang, r = j.weekdays;
		j = j.months;
		b = {
			a : r[f].substr(0, 3),
			A : r[f],
	...
	function Cc() {
		...
		Tb = a ? Date.UTC : function(b, c, d, e, f, g) {
			return (new Date(b, c, da(d, 1), da(e, 0), da(f, 0), da(g, 0)))
					.getTime()
		};
	...
	function Xc(a) {
		...
		function d(l) {
			var h = l.type || A.defaultSeriesType, s = bb[h], q = t.hasRendered;
			if (q)
				if (Ea && h == "column")
					s = Ic;
				else if (!Ea && h == "bar")
		...
		function g(l) {
			...
			for (h = 0; h < Ka.length; h++)
				if (Ka[h].options.id == l)
					return Ka[h];
			for (h = 0; h < ya.length; h++)
				if (ya[h].options.id == l)
					return ya[h];
			for (h = 0; h < ya.length; h++) {
				q = ya[h].data;
		...
		function j() {
			A.alignTicks !== false && p(Ka, function(l) {
				l.adjustTickAmount()
			})
		}
		function r() {
			var l = [];
			p(ya, function(h) {
				l = l.concat(Wb(h.data, function(s) {
					return s.selected
				}))
			});
			return l
		...
		function y(l) {
			var h = Ba.lang;
			t.toolbar.add("zoom", h.resetZoom, h.resetZoomTitle, function() {
				na(t, "selection", {
					resetSelection : true
				}, y);
				t.toolbar.remove("zoom")
			});
			!l || l.resetSelection ? p(Ka, function(s) {
				s.setExtremes(null, null, false)
			}) : p(l.xAxis.concat(l.yAxis), function(s) {
				var q = s.axis;
				if (t.tracker[q.isXAxis ? "zoomX" : "zoomY"])
					q.setExtremes(s.min, s.max, false)
			});
			c()
		}
		function R() {
			var l = a.title, h = a.subtitle;
			if (!t.titleLayer) {
		...
		function E() {
			var l = true;
			for ( var h in t.resources)
				t.resources[h] || (l = false);
			l && K()
		}
		function K() {
			k();
		...
		function Wa() {
			var l, h = a.labels, s = a.credits;
			l = 2 * (A.borderWidth || 0) + (A.shadow ? 8 : 0);
			Jc.drawRect(l / 2, l / 2, mb - l, Ga - l, A.borderColor,
					A.borderWidth, A.borderRadius, A.backgroundColor, A.shadow);
			Jc.drawRect(ga, I, sa, ka, null, null, null, A.plotBackgroundColor,
					null, Yb);
			(new pa("plot-border", oa, null, {
				zIndex : 4
			})).drawRect(ga, I, sa, ka, A.plotBorderColor, A.plotBorderWidth,
					null, null, A.plotShadow);
		...
		function Fa(l, h) {
				...
				p(
						...
						function(u) {
							...
							p(
									[ "xAxis", "yAxis" ],
									function(J) {
										if ((J == "xAxis" && L || J == "yAxis"
												&& !L)
												&& (u.options[J] == h.index || u.options[J] === ma
														&& h.index == 0)) {
											u[J] = Ya;
											$b.push(u);
											n = true
							...
							if (!u.visible && A.ignoreHiddenSeries)
								n = false;
							if (n) {
								var x;
								if (!L) {
			...
			function U() {
				...
				for ( var m = Ba.global.useUTC, n = 1E3 / Sa, u = 6E4 / Sa, x = 36E5 / Sa, z = 864E5 / Sa, J = 6048E5 / Sa, G = 2592E6 / Sa, ja = 31556952E3 / Sa, ba = [
						...
						[ "day", z, [ 1, 2 ] ], [ "week", J, [ 1, 2 ] ],
						[ "month", G, [ 1, 2, 3, 4, 6 ] ], [ "year", ja, null ] ], gb = ba[6], N = gb[1], Z = gb[2], $a = 0; $a < ba.length; $a++) {
					gb = ba[$a];
					N = gb[1];
					Z = gb[2];
					if (ba[$a + 1]) {
				...
				Z.setMilliseconds(0);
				if (N >= n)
					Z.setSeconds(N >= u ? 0 : ba * Za(Z.getSeconds() / ba));
				if (N >= u)
					Z[Dc](N >= x ? 0 : ba * Za(Z[oc]() / ba));
				if (N >= x)
					Z[Ec](N >= z ? 0 : ba * Za(Z[mc]() / ba));
				if (N >= z)
					Z[pc](N >= G ? 1 : ba * Za(Z[Ib]() / ba));
				if (N >= G) {
			...
			function za() {
				if (!hb && !Ha) {
					var m = Fb, n = fa.length;
					Fb = Cb[wb];
					if (n < Fb) {
						for (; fa.length < Fb;)
							fa.push(fa[fa.length - 1] + Ia);
						yb *= (n - 1) / (Fb - 1)
					}
					if (Qa(m) && Fb != m)
						Ya.isDirty = true
			...
			function P() {
				...
				if (!Ha && !ac) {
					m = ha - W || 1;
					if (!Qa(h.min) && Qc && (Ma < 0 || !Lc))
						W -= m * Qc;
					if (!Qa(h.max) && Rc && (Ta > 0 || !Mc))
						ha += m * Rc
				}
				Ia = Ha || W == ha ? 1 : h.tickInterval == "auto" ? (ha - W)
						* h.tickPixelInterval / sb : h.tickInterval;
				if (!hb && h.tickInterval == "auto")
					Ia = ia(Ia);
				uc = h.minorTickInterval == "auto" && Ia ? Ia / 5
						: h.minorTickInterval;
				ca();
				yb = sb / (ha - W || 1);
				Cb || (Cb = {
				...
				if (!hb && fa.length > Cb[wb])
					Cb[wb] = fa.length;
				if (!L)
					for (n in eb)
						for (u in eb[n])
							eb[n][u].cum = eb[n][u].total;
				Ya.isDirty = W != x || ha != z
			}
			function Aa(m, n, u) {
				u = da(u, true);
			...
			function Q() {
				var m = h.title, n = h.alternateGridColor, u = h.minorTickWidth, x = h.lineWidth, z, J;
				z = $b.length && Qa(W) && Qa(ha);
				Eb.clear();
				Nc.clear();
				if (z) {
					n && p(fa, function(G, ja) {
						if (ja % 2 == 0 && G < ha)
							xa(G, fa[ja + 1] !== ma ? fa[ja + 1] : ha, n)
					});
					p(wc, function(G) {
					...
					p(fa, function(G, ja) {
						...
						D(G, h.tickPosition, h.tickColor, h.tickWidth,
								h.tickLength,
								!(G == W && !h.showFirstLabel || G == ha
										&& !h.showLastLabel), ja)
					});
					p(vc, function(G) {
				...
				if (x) {
					u = ga + (la ? sa : 0) + Ua;
					z = Ga - cb - (la ? ka : 0) + Ua;
					Eb.drawLine(Y ? ga : u, Y ? z : I, Y ? mb - fb : u, Y ? z
							: Ga - cb, h.lineColor, x)
				}
				if (m && m.enabled && m.text) {
					...
					u = (Y ? I + ka : ga)
							...
							* (la ? -1 : 1)
							* m.margin
							- (Ra ? parseInt(m.style.fontSize
									|| m.style.font.replace(/^[a-z ]+/, "")) / 3
									: 0);
					Eb.addText(m.text, Y ? x : u + (la ? sa : 0) + Ua, Y ? u
							- (la ? ka : 0) + Ua : x, m.style, m.rotation || 0,
							{
								low : "left",
								middle : "center",
								high : "right"
			...
			var L = h.isX, la = h.opposite, Y = Ea ? !L : L, eb = {
				bar : {},
				column : {},
				area : {},
				areaspline : {}
			...
			s();
			var Ya = this, hb = h.type == "datetime", Ua = h.offset || 0, wb = L ? "x"
					: "y", sb = Y ? sa : ka, yb, bc = Y ? ga : cb, Eb = new pa(
					"axis-layer", oa, null, {
						zIndex : 7
					}), Nc = new pa("grid-layer", oa, null, {
				zIndex : 1
			}), Ma, Ta, $b, Oc, Pc, ha = null, W = null, Qc = h.minPadding, Rc = h.maxPadding, Lc, Mc, ac, Sc = h.events, xc, wc = h.plotBands
					|| [], vc = h.plotLines || [], Ia, uc, Mb, fa, Fb, cc = h.labels.formatter, Ha = h.categories
					|| L && l.columnCount, xb = h.reversed, zb = Ha
					&& h.tickmarkPlacement == "between" ? 0.5 : 0;
			if (Ea && L && xb === ma)
				xb = true;
			la || (Ua *= -1);
			if (Y)
		...
		function ob(l, h) {
			function s(o) {
				...
				if (!o.pageX)
					o.pageX = o.clientX
							+ (va.documentElement.scrollLeft || va.body.scrollLeft);
				if (!o.pageY)
					o.pageY = o.clientY
							+ (va.documentElement.scrollTop || va.body.scrollTop);
				return o
			}
			function q(o) {
				var Q = {
				...
				p(Ka, function(L) {
					var la = L.translate, Y = L.isXAxis, eb = Ea ? !Y : Y;
					Q[Y ? "xAxis" : "yAxis"].push({
						axis : L,
						value : la(eb ? o.pageX - lb.x - ga : ka - o.pageY
								+ lb.y + I, true)
					})
				});
				return Q
			...
			function M() {
				...
				ib.onmousemove = function(o) {
					...
					o.returnValue = false;
					if (Nb) {
						za = Math.sqrt(Math.pow(X - o.pageX, 2)
								+ Math.pow(ca - o.pageY, 2)) > 10;
						if (S) {
							var Q = o.pageX - X;
							ra(P, {
								width : Da(Q) + F,
				...
				ib.onmouseup = function() {
					...
					if (P) {
						var Q = {
							xAxis : [],
							yAxis : []
						}, L = P.offsetLeft, la = P.offsetTop, Y = P.offsetWidth, eb = P.offsetHeight;
						if (za) {
							p(
									Ka,
									function(Ya) {
										var hb = Ya.translate, Ua = Ya.isXAxis, wb = Ea ? !Ua
												: Ua, sb = hb(wb ? L : ka - la
												- eb, true);
				...
				ib.onmouseout = function(o) {
					o = o || Pa.event;
					if ((o = o.relatedTarget || o.toElement) && o != dc
							&& o.tagName != "AREA") {
						D();
						l.mouseIsDown = Nb = za = false
					}
			...
			function ea(o) {
				var Q = l.hoverPoint, L = l.hoverSeries;
				if (L) {
					Q
							|| (Q = L.tooltipPoints[Ea ? o.pageY - lb.y - I
									: o.pageX - lb.x - ga]);
					if (Q && Q != U) {
			...
			function xa() {
				...
				if (!Ra)
					dc.src = "data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="
			}
			function D() {
				Ab && Ab.hide();
				if (l.hoverSeries) {
			...
			function ia(o) {
				var Q = 0, L, la = ib.childNodes;
				for (L = 0; L < la.length; L++)
					if (la[L].isLegendArea) {
						Q = L + 1;
						break
					}
				ib.insertBefore(o, la[Q])
			}
			var U, X, ca, za, P, Aa = A.zoomType, w = /x/.test(Aa), qa = /y/
					.test(Aa), S = w && !Ea || qa && Ea, Na = qa && !Ea || w
					&& Ea;
			xa();
			if (h.enabled)
				l.tooltip = Ab = jb(h);
		...
		function jb(l) {
			function h(ia, U) {
				var X = ia.tooltipPos;
				U = ia.series;
				var ca = l.borderColor || ia.color || U.color || "#606060", za = t.inverted, P, Aa, w, qa = ea.offsetHeight;
				w = ia.tooltipText;
				M = U;
				P = X ? X[0] : za ? sa - ia.plotY : ia.plotX;
				X = X ? X[1] : za ? ka - ia.plotX : ia.plotY;
				if (P >= 0 && P <= sa && X >= 0 && X <= ka)
					Aa = true;
				if (w === false || !Aa)
					q();
				else {
					...
					if (Aa > (D.w || 0) + 20 || Aa < (D.w || 0) - 20 || w > D.h
							|| D.c != ca || qa != ea.offsetHeight) {
						D.clear();
						D.drawRect(xa / 2, xa / 2, Aa + 20, w, ca, xa,
								l.borderRadius, l.backgroundColor, l.shadow);
						H(D, {
							w : Aa,
							h : w,
							c : ca
			...
			function s(ia, U) {
				var X = kb.style.visibility == db, ca = X ? ia
						: (kb.offsetLeft + ia) / 2;
				...
				yc = Da(ia - ca) > 1 || Da(U - X) > 1 ? function() {
					s(ia, U)
				} : null
			}
			function q() {
				if (kb)
					kb.style.visibility = db;
				M && M.drawPointState()
			}
			var M, ea, xa = l.borderWidth, D;
			kb = V(Va, null, {
				position : wa,
				visibility : db,
				overflow : db,
		...
		var nb = function(l) {
			function h(Aa) {
				...
				p(za, function(w) {
					if (w.options.showInLegend) {
						var qa = w.options.legendType == "point" ? w.data
								: [ w ];
						p(qa, function(S) {
							S.simpleSymbol = /(bar|pie|area|column)/
									.test(w.type);
							S.legendItem = M = V("li", {
								innerHTML : q.labelFormatter.call(S),
								className : S.visible ? "" : Ob
							}, null, D.firstChild);
							if (S.options && S.options.showCheckbox)
								S.checkbox = V("input", {
									type : "checkbox",
									checked : S.selected,
									defaultChecked : S.selected
								}, q.itemCheckboxStyle, M);
							Oa(M, "mouseover", function() {
								S.setState("hover")
							});
							Oa(M, "mouseout", function() {
								S.setState()
							});
							Oa(M, "click", function(Na) {
								Na = Na.target;
								var o = "legendItemClick", Q = function() {
									S.setVisible()
								};
								if (Na.tagName == "INPUT")
									na(S, "checkboxClick", {
									...
									S.firePointEvent ? S.firePointEvent(o,
											null, Q) : na(S, o, null, Q)
							});
							U.push(S)
						})
					}
			...
			function s(Aa) {
				...
				if (za.length) {
					if (q.borderWidth || q.backgroundColor)
						X.drawRect(D.offsetLeft, D.offsetTop, D.offsetWidth,
								D.offsetHeight, q.borderColor, q.borderWidth,
								q.borderRadius, q.backgroundColor, q.shadow);
					p(
							U,
							function(w) {
								if (w.legendItem) {
									var qa = w.legendItem, S = D.offsetLeft
											+ qa.offsetLeft;
									qa = D.offsetTop + qa.offsetTop
											+ qa.offsetHeight / 2;
									var Na = w.legendItem.className == Ob, o = Na ? q.itemHiddenStyle.color
											: w.color;
									!w.simpleSymbol
											&& w.options
											&& w.options.lineWidth
											&& X.drawLine(S, qa, S + xa, qa, o,
													w.options.lineWidth);
									if (w.simpleSymbol)
										X.drawRect(S, qa - 6, 16, 12, null, 0,
												2, o);
									else if (w.options && w.options.marker
											&& w.options.marker.enabled)
										w.drawMarker(X, S + xa / 2, qa, aa(
												w.options.marker, Na ? {
													fillColor : o,
													lineColor : o
												} : null))
					...
					if (ib) {
						ca = V("area", {
							shape : "rect",
							isLegendArea : true,
							coords : [ D.offsetLeft - ga, D.offsetTop - I,
									D.offsetLeft + D.offsetWidth - ga,
									D.offsetTop + D.offsetHeight - I ]
									.join(",")
						});
						ub.insertAtFront(ca);
						...
						D.onmouseout = ca.onmouseout = function(w) {
							w = w || Pa.event;
							if ((w = w.relatedTarget || w.toElement)
									&& (w == dc || w.tagName == "AREA"
											&& w != ca))
								ra(D, {
									zIndex : 7
			...
			if (q.enabled) {
				var M, ea = q.layout, xa = q.symbolWidth, D, ia = "#" + oa.id
						+ " .highcharts-legend li", U = [], X = new pa(
						"legend", oa, null, {
							zIndex : 7
						}), ca, za = l.series, P = q.reversed;
				this.dom = D = V(Va, {
		...
		a = aa(Ba, a);
		var A = a.chart, T = A.margin;
		T = typeof T == "number" ? [ T, T, T, T ] : T;
		var I = T[0], fb = T[1], cb = T[2], ga = T[3], Xa, rb, oa, qc, mb, Ga;
		C();
		var t = this;
		T = A.events;
		var zc, ib, Ab, Nb, Jc = new pa("chart-background", oa), vb, sc, ka, sa, ub, dc, Jb, lb = Hc(oa), Ub = A.showAxes, Ka = [], Cb, ya = [], Yb, Ea, yc, kb;
		fc = Bb = 0;
		Oa(Pa, "resize", i);
		Oa(Pa, "unload", La);
		if (T)
	...
	var ma, va = document, Pa = window, ta = Math, O = ta.round, Za = ta.floor, Lb = ta.max, Da = ta.abs, gc = ta.cos, hc = ta.sin, B = navigator.userAgent, Ra = /msie/i
			.test(B)
			&& !Pa.opera, cd = /AppleWebKit/.test(B), kc, bd = 0, Bb, fc, Uc = {}, rc = 0, Sa = 1, Qb, Va = "div", wa = "absolute", Xb = "relative", db = "hidden", Ob = "highcharts-"
			+ db, ec = "visible", F = "px", Tb, oc, mc, nc, Ib, Rb, Sb, Dc, Ec, pc, Fc, Gc, ua = (B = Pa.HighchartsAdapter)
			|| {}, p = ua.each, Wb = ua.grep, qb = ua.map, aa = ua.merge, Pb = ua.hyphenate, Oa = ua.addEvent, na = ua.fireEvent, Db = ua.animate, Ac = ua.getAjax, bb = {};
	if (!B && Pa.jQuery) {
		var pb = jQuery;
		p = function(a, b) {
	...
	pa.prototype = {
		...
		drawSymbol : function(a, b, c, d, e, f, g) {
			...
			if (a == "square") {
				...
				k.test(a) ? V("img", {
					onload : function() {
						var j = this, r = Uc[j.src] || [ j.width, j.height ];
						ra(j, {
							left : O(b - r[0] / 2) + F,
							top : O(c - r[1] / 2) + F,
							visibility : ec
		...
		addText : function(a, b, c, d, e, f) {
			if (a || a === 0) {
				...
				for ( var R in d)
					r += Pb(R) + ":" + d[R] + ";";
				p([ "MozTransform", "WebkitTransform", "transform" ], function(
						Wa) {
					if (Wa in k.style)
						j = Wa
				});
				if (!e || j) {
					...
					R = g.width;
					y = y / 3 || 3;
					var E = f == "left", K = f == "right", C = E ? b : b - R
							* e;
					b = K ? b : b + R * e;
					E = E ? c : c - R * d;
					c = K ? c : c + R * d;
					...
					g.svg += '<g_vml_:line from="'
							...
							+ a + '" style="v-text-align:' + f + ";" + r
							+ '"/></g_vml_:line>'
				} else {
					i = true;
					g.svg += '<g><text transform="translate(' + b + "," + c
							+ ") rotate(" + (e || 0) + ')" style="fill:' + v
							+ ";text-anchor:" + {
								left : "start",
								center : "middle",
								right : "end"
	...
	ic.prototype = {
		...
		select : function(a, b) {
			var c = this, d = c.series, e = d.chart, f, g, i = da(c.stateLayer,
					d.singlePointLayer, e.singlePointLayer);
			c.selected = a = da(a, !c.selected);
			d.isDirty = true;
			c.firePointEvent(a ? "select" : "unselect");
			i && i.clear();
			p(e.series, function(k) {
				f = k.stateLayers;
		...
		remove : function(a) {
			var b = this, c = b.series, d = c.chart, e = c.data;
			a = da(a, true);
		...
		importEvents : function() {
			if (!this.hasImportedEvents) {
				var a = this, b = aa(a.series.options.point, a.options);
				b = b.events;
				var c;
				a.events = b;
				for (c in b)
	...
	Ja.prototype = {
		...
		getData : function(a) {
			var b = this, c = b.chart, d = "series" + rc++;
			if (!a.data && a.dataURL) {
				c.addLoading(d);
				Ac(a.dataURL, function(e) {
					b.dataLoaded(e);
					c.clearLoading(d)
				})
			} else
		...
		dataLoaded : function(a) {
			var b = this, c = b.chart, d = b.options, e = [ "" ], f = d.dataParser, g = {}, i;
			if (d.dataURL && !f)
				f = function(k) {
					return eval(k)
				};
		...
		remove : function(a) {
			...
			if (!b.isRemoving) {
				b.isRemoving = true;
				na(b, "remove", null, function() {
					Ca(b.layerGroup.div);
					p(b.areas, function(d) {
						Ca(d)
					});
					Ca(b.legendItem);
					b.legendItem = null;
					p(c.series, function(d, e) {
		...
		setTooltipPoints : function(a) {
			var b = this, c = b.chart, d = c.inverted, e = [], f = d ? c.plotHeight
					: c.plotWidth, g, i, k = [];
			if (a)
				b.tooltipPoints = null;
			p(b.segments, function(j) {
				e = e.concat(j)
			});
			if (b.xAxis.reversed)
				e = e.reverse();
			p(
					...
					function(j, r) {
						...
						for (i = j.high = e[r + 1] ? Za((j.plotX + (e[r + 1] ? e[r + 1].plotX
								: f)) / 2)
								: f; g <= i;)
							k[d ? f - g++ : g++] = j
					});
		...
		drawLine : function(a) {
			var b, c = this, d = c.options, e = c.chart, f = d.animation
					&& c.animate, g = c.stateLayers[a], i = d.lineColor
					|| c.color, k = d.fillColor == "auto" ? Bc(c.color)
					.setOpacity(d.fillOpacity || 0.75).get() : d.fillColor, j = e.inverted, r = (j ? 0
					: e.plotHeight)
					- c.yAxis.translate(0);
			if (a)
				d = aa(d, d.states[a]);
			...
			p(c.segments, function(v) {
				...
				p(v, function(E, K) {
					if (K && d.step) {
						K = v[K - 1];
						y.push(j ? e.plotWidth - K.plotY : E.plotX,
								j ? e.plotHeight - E.plotX : K.plotY)
					}
					y.push(j ? e.plotWidth - E.plotY : E.plotX,
							j ? e.plotHeight - E.plotX : E.plotY)
				});
				if (/area/.test(c.type)) {
					...
					if (d.stacking && c.type != "areaspline")
						for (b = v.length - 1; b >= 0; b--)
							R.push(v[b].plotX, v[b].yBottom);
					else
						R.push(j ? r : v[v.length - 1].plotX, j ? e.plotHeight
								- v[v.length - 1].plotX : r,
								j ? r : v[0].plotX, j ? e.plotHeight
										- v[0].plotX : r);
		...
		drawPoints : function(a) {
			var b = this, c = b.stateLayers[a];
			a = b.options;
			var d = a.marker;
			a = b.data;
			var e = b.chart, f = e.inverted;
			d.enabled
					&& p(a, function(g) {
						if (g.plotY !== ma)
							b.drawMarker(c,
									f ? e.plotWidth - g.plotY : g.plotX,
									f ? e.plotHeight - g.plotX : g.plotY, aa(d,
											g.marker));
		...
		drawDataLabels : function() {
			if (this.options.dataLabels.enabled) {
				var a = this, b, c, d = a.data, e = a.options.dataLabels, f, g = a.dataLabelsLayer, i = a.chart, k = i.inverted, j = a.type, r = j == "pie", v;
				if (g)
					g.clear();
				else
					a.dataLabelsLayer = g = new pa("data-labels",
							a.layerGroup.div, null, {
								zIndex : 1
							});
				e.style.color = e.color == "auto" ? a.color : e.color;
				p(d, function(y) {
					var R = y.plotX, E = y.plotY, K = y.tooltipPos;
					f = e.formatter.call({
						x : y.x,
						y : y.y,
						series : a,
					...
					if (r) {
						if (!y.dataLabelsLayer)
							y.dataLabelsLayer = new pa("data-labels",
									y.layer.div, null, {
										zIndex : 3
									});
						g = y.dataLabelsLayer
					...
					if (f)
						g[r ? "drawText" : "addText"](f, b, c, e.style,
								e.rotation, v)
				});
				r || g.strokeText()
			}
		...
		drawPointState : function(a, b, c) {
			var d = this.chart, e = d.inverted, f = b == "hover";
			c = c || d.singlePointLayer;
			var g = this.options;
			if (f) {
				if (!c)
					c = d.singlePointLayer = new pa("single-point",
							d.plotLayer.div, null, {
								zIndex : 3
							});
				c.clear()
			}
			if (b) {
				...
				if (f && b.radius === ma)
					b.radius = i.radius + 2;
				if ((f = aa(g.marker, a.marker, i, b)) && f.enabled)
					this.drawMarker(c, e ? d.plotWidth - a.plotY : a.plotX,
							e ? d.plotHeight - a.plotX : a.plotY, f)
			}
		},
		destroy : function() {
			var a = this, b;
		...
		setVisible : function(a, b) {
			var c = this, d = c.chart, e = c.layerGroup, f = c.legendItem, g = c.areas, i = c.visible;
			...
			d.options.chart.ignoreHiddenSeries && c.options.stacking
					&& p(d.series, function(k) {
						if (k.options.stacking && k.visible)
							k.isDirty = true
					});
			b !== false && d.redraw();
			na(c, a ? "show" : "hide")
		...
		getAreaCoords : function() {
			var a = this, b = this.chart, c = b.inverted, d = b.plotWidth, e = b.plotHeight, f = a.xAxis.reversed, g, i = b.options.tooltip.snap, k = [];
			p(
					a.splinedata || a.segments,
					function(j, r) {
						if ((g = j.length > 1 && j[0].x > j[1].x) && !f || f
								&& !g)
							j = j.reverse();
						var v = [], y = [], R = [];
						p(
								[ y, R ],
								function(E) {
									for ( var K = 0, C = 0, Wa, La, Fa = [ j[0] ], Kb = E == y ? 1
											: -1, ob, jb, nb, A, T, I, fb; j[C];) {
										if (j[C].plotX > j[K].plotX + i
												|| C == j.length - 1) {
											Wa = j[C];
											La = j.slice(K, C - 1);
											p(La, function(cb) {
									...
									Fa[Fa.length - 1] != j[j.length - 1]
											&& Fa.push(j[j.length - 1]);
						...
						for (r = 0; r < y.length; r++)
							v.push(c ? d - y[r][1] : y[r][0], c ? e - y[r][0]
									: y[r][1]);
						for (r = R.length - 1; r >= 0; r--)
							v.push(c ? d - R[r][1] : R[r][0], c ? e - R[r][0]
									: R[r][1]);
		...
		createArea : function() {
			if (this.options.enableMouseTracking !== false) {
				var a, b = this, c = b.options, d = b.chart, e = d.tracker, f = b
						.getAreaCoords(), g = [], i = b.areas, k;
				i && p(i, function(j) {
					Ca(j)
				});
				p(f, function(j) {
	...
	B = ab(Ja, {
		...
		getSplineData : function() {
			...
			p(a.segments, function(e) {
				...
				if (f.length > 1)
					d = O(Lb(b.plotWidth, f[f.length - 1].clientX
							- f[0].clientX) / 3);
	...
	var Vb = ab(
			Ja,
			{
				...
				translate : function() {
					...
					var e = a.options, f = a.data, g = b.inverted, i = b.plotWidth, k = b.plotHeight, j = a.closestPoints;
					j = Da(f[1] ? f[j].plotX - f[j - 1].plotX : g ? k : i);
					var r = j * e.groupPadding, v = j - 2 * r;
					v = v / c;
					var y = e.pointWidth;
					e = Qa(y) ? (v - y) / 2 : v * e.pointPadding;
					var R = da(y, v - 2 * e);
					b = (b.options.xAxis.reversed ? c - a.columnIndex
							: a.columnIndex) || 0;
					var E = -(j / 2) + r + b * v + e, K = a.yAxis.translate(0);
					p(f, function(C) {
						C.plotX += E;
						C.w = R;
						C.y0 = (g ? i : k) - K;
				...
				drawPoints : function(a) {
					...
					p(g, function(k) {
						if (k.plotY !== ma)
							i.drawRect(f ? k.h >= 0 ? d.plotWidth - k.plotY
									- k.h : d.plotWidth - k.plotY : k.plotX,
									f ? d.plotHeight - k.plotX - k.w
											: k.h >= 0 ? k.plotY : k.plotY
													+ k.h, f ? Da(k.h) : k.w,
									f ? k.w : Da(k.h), c.borderColor,
									c.borderWidth, c.borderRadius, k.color
											|| b.color, c.shadow);
				...
				drawPointState : function(a, b, c) {
					var d = this, e = d.chart, f = d.options, g = a ? a.options
							: null, i = e.inverted;
					c = c || d.singlePointLayer;
					if (b == "hover") {
						if (!c)
					...
					if (b && this.options.states[b]) {
						b = aa(f, f.states[b], g);
						c.drawRect(i ? e.plotWidth - a.plotY - a.h : a.plotX,
								i ? e.plotHeight - a.plotX - a.w : a.plotY,
								i ? a.h : a.w, i ? a.w : a.h, b.borderColor,
								b.borderWidth, b.borderRadius, Bc(
				...
				getAreaCoords : function() {
					...
					p(
							this.data,
							function(d) {
								var e = Lb(Da(d.h), 3) * (d.h < 0 ? -1 : 1), f = c ? b.plotWidth
										- d.plotY - e
										: d.plotX, g = c ? b.plotHeight
										- d.plotX - d.w : d.plotY, i = g
										+ (c ? d.w : e);
								e = f + (c ? e : d.w);
								if (!c && Da(e - f) < 1)
									e = f + 1;
	...
	B = ab(ic, {
		...
		init : function() {
			...
			var a = this, b = a.series, c = b.chart.options.colors;
			H(a, {
			...
			if (Bb >= c.length)
				Bb = 0;
			if (!a.layer)
				a.layer = new pa("pie", b.layerGroup.div);
			b = function() {
				a.slice()
			};
			Oa(a, "select", b);
		...
		setVisible : function(a) {
			var b = this, c = b.layer, d = b.legendItem;
	...
	B = ab(
			Ja,
			{
				...
				translate : function() {
					var a = 0, b = this, c = -0.25, d = b.options, e = d.slicedOffset, f = d.center, g = b.chart;
					b = b.data;
					var i = 2 * ta.PI, k;
					f.push(d.size);
					f = qb(f, function(j, r) {
						return /%$/.test(j) ? g["plot"
								+ (r ? "Height" : "Width")]
								* parseInt(j) / 100 : j
					});
					p(b, function(j) {
				...
				drawPointState : function(a, b, c) {
					...
					if (a) {
						...
						if (b == "hover") {
							if (!c)
								c = a.stateLayer = new pa("single-point",
										a.layer.div);
							c.clear()
						}
						if (b && d.options.states[b]) {
							b = aa(e, e.states[b]);
							this.drawPoint(a, c.getCtx(), b.color || a.color,
									b.brightness)
						}
					}
					d.hoverPoint && d.hoverPoint.stateLayer
							&& d.hoverPoint.stateLayer.clear();
					d.hoverPoint = a
				},
				drawPoint : function(a, b, c, d) {
					var e = this.options, f = a.sliced ? a.centerSliced
							: a.center, g = f[0];
					f = f[1];
					var i = a.size, k = e.borderWidth, j = Ra
							&& a.percentage == 100 ? a.start : a.end;
					if (a.y > 0) {
