	...
	var fs = {
		...
		add_data : function(targetid){					//添加数据到容器，添加事件，初始化下一层次容器
			...
			for(var i = 0 ; i < dat.length ; i++){
				...
				region = dat[i];
				html.push("<span title='"+region.regionName+"' name='"+pid+"' style='width:"+span_width+"px;white-space:nowrap;float:left;'>");
				html.push("<input type='checkbox' value='" + region.regionCode + "'>");
		...
		init_event : function(){		//绑定已选择框中checkbox的事件，确定，取消事件响应
			...
			$("#_ok").click(function(){
				...
				if(jQuery.isFunction(param.callback)){
					param.callback(vals.substring(1),txts.substring(1));
				}else{
		...
		add_span_event : function(targetid){	//新生成的元素集合添加span的单击事件响应
			...
			spans.children("span").click(function(e){
				if (maxlev > thislevel){
					...
					$(this).parent().css({"background":"#D7D7D7","border":"1px dashed  #D7D7D7","margin":"-1","color":"#000000"});
		...
		init_style : function() {	//初始化css
			...
			var d_css = "{width:"+_width+"px;margin:"+_margin+"px;padding-left:10px;height:auto;background-color:#ffffff;float:left;border-bottom: 1px dashed #D7D7D7;}";
			css.push("dc "+d_css);
