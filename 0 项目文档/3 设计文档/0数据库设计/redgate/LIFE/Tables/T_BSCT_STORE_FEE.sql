CREATE TABLE "LIFE".t_bsct_store_fee (
  ...
  sel_state NUMBER(2),
  CONSTRAINT pk_t_bsct_store_fee PRIMARY KEY ("ID")
);
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.capital_type IS '1：现金  2：商城币  3：积分
';
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.fee_type IS '1：业务平台计费   2：统一清算计费
';
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.fee_mode IS '0：不收取  2：单笔计算、按周期收取  3：按周期汇总计算并收取
';
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.fee_method IS '1:按固定金额收取  2:按金额固定百分比收取
';
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.fee_basicflag IS '0：金额  1：笔数';
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.fee_levelflag IS '空格：表示只有一档；0：套档，1：分层；最多可分5层/档。
本字段没有输入的情况下，只有一档费用计算参数。
';
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.status IS '1：正常  0： 删除   ';
COMMENT ON COLUMN "LIFE".t_bsct_store_fee.sel_state IS '高阳状态';