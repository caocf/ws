...
public class ItemManageService {
	...
	public Page<ItemSale> listItemSale(ItemSale itemSale, int pageNo, int pageSize, String ids) throws SQLException {
		...
		//sql.append(" and sa.item_mode = 0 ");// 实物商品
		...
		// 礼品卡功能
		if (null != itemSale.getIseckill() && 2L == itemSale.getIseckill()) {
			sql.append(" and sa.iseckill in (2,4) ");
		}
		//礼品卡兑换商品
		if (null != itemSale.getIseckill() && -1 == itemSale.getIseckill()) {
			sql.append(" and sa.iseckill = 2 ");
		}
		//礼品卡
		if (null != itemSale.getIseckill() && -2 == itemSale.getIseckill()) {
			sql.append(" and sa.iseckill = 4 ");
		}
		// if (StringUtils.isNotEmpty(itemSale.getSaleAreaCode())) {
		// sql.append(" join t_item_sale_area_link isal on isal.sale_id = sa.id and isal.REGION_CODE = ? ");
		// params.add(itemSale.getSaleAreaCode());
	...
	@Transactional
	public ItemSale auditItemSale(ItemSale vo, AuditStep step) throws Exception {
	...
	public void saveItemSalePayment(ItemSale itemSale) {
		...
		itemSalePayment.setDeliveryPay(itemSale.getDeliveryPay());
	...
	public String auditItem(ItemSale sale, AuditStep step) {
		...
		try {
			RequestUrl.post(appConfig.getCacheRefreshUrl() + appConfig.getItemUrl() + sale.getId());
			log.info("商品更新，调用缓存刷新接口成功.");
		}
		catch (Exception e) {
			log.error("商品缓存重置异常：" + e.getMessage());
			logUtils.logOther("商品缓存重置异常", "商品" + sale.getId() + "更新，调用缓存刷新接口失败");
		}
	...
	public ItemSale saveItemSale(ItemSale itemSale) {
		/**
		 * 临时调用缓存刷新的接口，后期调整到AOP类的AfterReturningAdviceRefreshUrl-->itemEdit()中拦截。
		 * 
		 * @modify_by macl
		 * @date 2013-10-25
		 */
		refreshCache(itemSale);
	 ...
	 * @param itemSale
	 */
	public void refreshCache(ItemSale itemSale) {
		try {
			RequestUrl.post(appConfig.getCacheRefreshUrl() + appConfig.getItemUrl() + itemSale.getId());
			log.info("商品更新，调用缓存刷新接口成功.");
		}
		catch (Exception e) {
			log.error("商品缓存重置异常：" + e.getMessage());
			logUtils.logOther("商品缓存重置异常", "商品" + itemSale.getId() + "更新，调用缓存刷新接口失败");
		}
	}
	...
	public List<ItemPriceTypeOnArea> getItemPriceTypeByAreaCodes(String areaCodes) throws SQLException {
		String sql = " select priceType.* , region.region_name regionName from "
		        ...
		        + "        ) order by it.id)  priceType , t_sys_region region " + "		where  region.region_code=priceType.area_Code ";
