...
<div id="search-menu">
    <ul>
        ...
        <ct:display model="sms_batch_list" btn="add_btn">
            <ht:menu-btn text="添加任务" url="/batch/add" type="add"/>
        </ct:display>
    </ul>
    <br style="clear: left" />
</div>
<div class="queryContainer" >
    <form name="queryForm" id="queryForm" action="smsbatchlist" method="get">
        <table>
            <tr>
    ...
    <div class="mainbox">
        ...
        <table class="datalist fixwidth">
            ...
            <tr>
                ...
                <td  nowrap="nowrap">
                <ct:display model="sms_batch_list" btn="view_btn">
                        <a href="view/batchList/${item.id}">查看</a>
                    </ct:display>
                    &nbsp;&nbsp;
                 <c:if test="${item.status==0 || item.status==2}">
                    <ct:display model="sms_batch_list" btn="mod_btn">
                        <a href="edit/${item.id}">修改</a>
                    </ct:display>
                    &nbsp;&nbsp;
                    <ct:display model="sms_batch_list" btn="del_btn">
                        <a href="#" onclick="deleteItem('delete/${item.id}/smsbatchlist');">删除</a>
                    </ct:display>
                   </c:if> 
                </td>
            </tr>
