            ...
            <tr>
                ...
                <td width="200">
                    <select name="lockStatus">
                        ...
                        <option value="0" <c:if test="${param.lockStatus == 2}">selected="selected"</c:if>>未锁定</option>
                        <option value="1" <c:if test="${param.lockStatus == 1}">selected="selected"</c:if>>已锁定</option>
                    </select>
                </td>
            </tr>
            <tr>
        ...
        <table class="datalist fixwidth">
            ...
            <tr>
                ...
                <td nowrap="nowrap" width="100"><ct:time source="${item.createTime}"/></td>
                 <td nowrap="nowrap" width="100">${item.lockStatusTypeName}</td>
                <td width="100" nowrap="nowrap">
                <c:if test="${item.lockStatus==1}">
                    <ct:display model="user_list" btn="mod_btn">
                           <a href="#" onclick="deleteItem('userUnlock?id=${item.id}');">解锁</a>
                    </ct:display>
                 </c:if>
                </td>
            </tr>
