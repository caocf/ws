   ...
   <script type="text/javascript">
	...
	$().ready(function() {
		...
		$(".item_audit").click(function() {
			var id = $(this).attr("id");
			showDialog("商品审核", "${ctx}/item/item/twoAuditing/"+id,function(doc){
    ...
    <input type="hidden" name="iseckillFlag" value="${iseckillFlag}" />
        <table>
           ...
           <td nowrap="nowrap">商品编号：</td>
             <td width="120"><input id="" type="text" name="id" title="商品编号" value="${param.id}" class="txt validate-number" style="width:100px"/></td>
             <td width="60">创建时间：</td>
                <td >
               ...
               <td nowrap="nowrap">商户编号：</td>
             <td width="120"><input type="text" name="storeId" title="商户编号" value="${param.storeId}" class="txt validate-number" style="width:100px"/></td>
                 <td width="60">商户名称：</td>
        ...
        <table class="datalist fixwidth">
            ...
            <tr>
                ...
                <td width="100" nowrap="nowrap">
                   ...
                   <ct:display model="twoaudit_list" btn="view_btn">
                        <a href="view/${item.id}">查看</a>
                    </ct:display>
                      <c:if test="${item.status==2}">
                      &nbsp; &nbsp;
                       <ct:display model="twoaudit_list" btn="audit_btn">
	                	  <a href="${ctx}/item/item/twoAuditing/${item.id}">审核</a>
	                	  &nbsp;&nbsp;
	                </ct:display>
                     </c:if>
                </td>
