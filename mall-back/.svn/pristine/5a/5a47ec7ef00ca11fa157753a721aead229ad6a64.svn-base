...
public class SmsActOnlineController {
	...
	public String list(SmsActOnline online, @RequestParam(value = "page", required = false, defaultValue = "1") int page, Model model)
	        throws SQLException {
	...
	@ResponseBody
	public Object createPost(@ModelAttribute("online") SmsActOnline online, HttpServletRequest request, BindingResult result) {
		...
		try {
			...
			// 提示并跳转
			logUtils.logAdd("短信业务配置", "新增短信业务配置，ACT_ID："+online.getActId());
			return JsonRespWrapper.success("配置成功", "/smsact/seconddevelop/online_query");

		}
		catch (Exception ex) {
			logUtils.logAdd("短信业务配置", "新增短信业务配置失败，ACT_ID："+online.getActId());
	...
	@ResponseBody
	public Object updatePost(@ModelAttribute("online") SmsActOnline online, HttpServletRequest request, BindingResult result) {
		online.setStartTime(online.getStartTime().replaceAll("-", "").replaceAll(" ", "").replaceAll(":", ""));
		online.setEndTime(online.getEndTime().replaceAll("-", "").replaceAll(" ", "").replaceAll(":", ""));
		online.setStatus(SmsActOnline.STATUS_0);
		try {
			...
			logUtils.logModify("短信业务配置", "修改短信业务配置，ACT_ID："+online.getActId());
			return JsonRespWrapper.success("操作成功！", backUrl);
		}
		catch (Exception ex) {
			logUtils.logModify("短信业务配置", "修改短信业务配置失败，ACT_ID："+online.getActId()+ex.getMessage());
			log.error(ex.getMessage());
			// 未知的错误消息，一般是exception catch后通知用户
			return JsonRespWrapper.error(ex.getMessage());
		}
	...
	public Object operate(@PathVariable Long id, @PathVariable Long status, @PathVariable String whereAbout, Model model) {
		...
		try {
			...
			logUtils.logAudit("短信业务审核出错",ex.getMessage() );
			log.error(ex.getMessage());
			// 未知的错误消息，一般是exception catch后通知用户
			return JsonRespWrapper.error(ex.getMessage());
		}
	...
	@ResponseBody
	public Object addRouter(@ModelAttribute("router") SmsActRouter router, HttpServletRequest request, BindingResult result) {
	...
	public Object deleteRouter(@PathVariable Long id) {
		...
		logUtils.logDelete("短信指令删除", "删除操作成功");
		return true;
	}
}
