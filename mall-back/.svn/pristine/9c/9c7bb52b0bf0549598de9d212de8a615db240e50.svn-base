			...
			<form:form method="post" id="fm"  
				...
				<div class="form">
					<div class="fields">
		                ...
		                <div class="field">
		                    <div class="label">礼品卡兑换礼品描述：</div>
		                    <div class="input">
		                    <c:if test="${empty giftRequired.remark}">暂无描述信息</c:if>
		                    ${giftRequired.remark}
		                    </div>
		                </div>
		                <c:forEach begin="1" end="${giftRequired.exchangeMode}" varStatus="index">
		                <div class="field">
							<div class="label">
								<label class="req">选择商品：</label>
							</div>
							<div class="input">
								<input name="item${index.index }" id="item${index.index }" onclick="selectItem(${index.index});"  class="small required" style="width:150px;height:15px;">
								<span class="error" id="advice-required-item${index.index }" style="display:none"></span>
								<input type="hidden" name="itemId" id="itemId${index.index }">
								<input type="hidden" name="storeId" id="storeId${index.index }">
							</div>
						</div>
		                </c:forEach>
						<div class="buttons">
