	...
	<div id="content">
		...
		<div class="box">
			...
			class="required-validate"
				action="${ctx}/item/vitual/audit/${id}"
				...
				<div class="form">
					<div class="fields">
							...
							<div class="radios">
								<div class="radio">
									<input type="radio" id="radio-1" name="status"
										checked="checked" value="3" /><label for="radio-1">通过</label>
									<input type="radio" id="radio-2" name="status" value="4" /><label
						...
						<div id="para" >
						  	...
						  	<div class="field">
			                    ...
			                    <div class="checkboxes" id="payType1"  <c:if test="${!payTypeFg }">style="display: none;"</c:if>/>
			                        <div class="checkbox">
			                            <input type="checkbox" id="checkbox-1" name="cashIdgoods"  value="1"  <c:if test="${itemSale.cashIdgoods == 1 }">checked="checked"</c:if>/><label for="checkbox-1">现金</label>
			                            <input type="checkbox" id="checkbox-2" name="coinIdgoods" value="1"  <c:if test="${itemSale.coinIdgoods == 1 }">checked="checked"</c:if>/><label for="checkbox-2">商城币</label>
			                        	<input type="checkbox" id="checkbox-3" name="scoreIdgoods"  value="1"  <c:if test="${itemSale.scoreIdgoods == 1 }">checked="checked"</c:if>/><label for="checkbox-3">积分</label>
			                        </div>
			                    </div>
						...
						<div class="field" id="numLimit"
							...
							<div class="input">
								<input id="userPerBuyNum" name="userPerBuyNum"
									...
									<c:if test="${itemSale.userPerBuyNum == null}">value="0"</c:if>
									<c:if test="${itemSale.userPerBuyNum != null}">value="${itemSale.userPerBuyNum }"</c:if> maxlength="8" /> 
	                   ...
	                   <div class="label label-radio">
	                    ...
	                    <div class="radios">
	                        <div class="radio">
	                        <input type="radio" id="radio-1" name="isView" class="validate-one-required" <c:if test="${itemSale.isView == 1  }">checked="checked" </c:if>  value="1"/><label for="radio-1">是</label>
