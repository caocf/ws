        ...
        <div class="form">
            <div class="fields">
                ...
                <div class="field">
                    ...
                    <div class="input"><c:if test="${sysAnnouncement.title == 0}">静态公告</c:if><c:if test="${sysAnnouncement.title == 1}">拉伸公告</c:if><c:if test="${sysAnnouncement.title !=0 && sysAnnouncement.title !=1}">${sysAnnouncement.title }</c:if></div>
                </div>
                
                
			<div <c:if test="${sysAnnouncement.pubDest == 3}"> style="display:none"</c:if>>
                <div class="field"  	>
                    <div class="label">
                        <label >公告内容：</label>
                    </div>
                ...
                <c:if test="${sysAnnouncement.pubDest == 3 && sysAnnouncement.title == 0}">
                	<div class="field">
	                    <div class="label noinput">公告图片：</div>
		                <div class="input">
		                    <ht:image webpath="${advisePath   }" type="advise" />	
		                </div>
               		</div>
                </c:if>
                
                <c:if test="${sysAnnouncement.pubDest == 3 && sysAnnouncement.title == 1}">
                	<div class="field">
	                    <div class="label noinput">公告图片：</div>
	                    <div class="input">
		                    <ht:image webpath="${advisePath   }" type="advise" />	
		                ...
		                <div class="input">
		                    <ht:image webpath="${adviseExtendPath   }" type="adviseExtend" />	
		                </div>
               		</div>
                </c:if>
