...
public class StoreService {
	...
	public Page<Store> listStore(Store store, int pageNo, int pageSize, String status, String ids) throws SQLException {
		...
		sql.append("select store.*,region.region_name areaName from t_store store ")
		        .append(" left join t_sys_region region on store.area_code = region.region_code ")
		        .append("  join t_store_settle settle on settle.store_id = store.id")
		        .append(" where 1 = 1");
		...
		if (StringUtils.isNotEmpty(store.getBaseShopName())) {
			...
			params.add("%" + store.getBaseShopName().trim() + "%");
		}
		...
		if (StringUtils.isNotEmpty(store.getAddress())) {
			...
			params.add("%" + store.getAddress().trim() + "%");
		}
		...
		sql.append(" and settle.effort_date <=" + TimeStamp.getTime(TimeStamp.YYYYMMDD));
		sql.append(" and settle.expiry_date >=" + TimeStamp.getTime(TimeStamp.YYYYMMDD));
		sql.append(" and settle.status = 3 ");
		sql.append(" and settle.sync_gy_flag = 3 ");
		// 获取已同步商户end
	 ...
	 */
	@Transactional
	public void addChannel(Store store, Long sysUserId, InputStream is) throws Exception {
	 ...
	 */
	@Transactional
	public String auditStore(Store store, AuditStep step) throws SQLException {
		String msg = "操作成功";
		// try {
	...
	public void saveSroreSettle(StoreSettle settle, InputStream is) throws Exception {
	 ...
	 * @param is
	 */
	public void saveSroreSettleContractFile(StoreSettle settle, String ext, InputStream is) throws Exception {
		this.addOrUpdataStoreSettle(settle);
		if (null != is) {
			bsFileService.dealBusiContractFile(is, ext, BsFileService.BS_CHANNEL_TREATY_KEY, settle.getStoreId(), false);
		}
	}

	/**
	 * 商户结算信息修改和商户协议附件修改。 2013-09-02
	 * 
	 * @author liujun
	 * @param settle
	 * @param is
	 */
	public void updateSroreSettleContractFile(StoreSettle settle, String ext, InputStream is) throws Exception {
		this.addOrUpdataStoreSettle(settle);
		if (null != is) {
			bsFileService.dealBusiContractFile(is, ext, BsFileService.BS_CHANNEL_TREATY_KEY, settle.getStoreId(), true);
		}
	}

	public void updateSroreSettle(StoreSettle settle, InputStream is) throws Exception {
		this.addOrUpdataStoreSettle(settle);
		if (null != is) {
	...
	@Deprecated
	public List<Object> saveSysFee(StoreFee storeFee) throws Exception {
		List<Object> lists = new ArrayList<Object>();
		String productionTypeName = storeFee.getProductionTypeName();
		Long storeId = storeFee.getStoreId();
		SysFee sysfee = new SysFee();
		sysfee.setStoreId(storeId);
		sysfee.setName(productionTypeName);
		sysfee.setValid(1L);
		sysfee.setSyncGyFlag(0L);
		SysFee sf = feeService.saveUpdateBrand(sysfee);
		SyncGYResponseBean bean = null;
		String msg = "同步成功";
		if (null != sf) {
			if (sf.getSyncGyFlag() != 0L) {
				bean = syncInterface.syncUpdateGoodsType(sf);
			} else {
				bean = syncInterface.syncAddGoodsType(sf);
			}
		}
		if (bean != null) {
			if (bean.isSyncSuccess()) {
				sf.setSyncGyFlag(1L);
				sf.setSyncGyTime(TimeStamp.getTime(TimeStamp.YYYYMMDDhhmmss));
				feeService.saveUpdateBrand(sf);
			}
		} else {
			msg = "同步失败。";
		}
