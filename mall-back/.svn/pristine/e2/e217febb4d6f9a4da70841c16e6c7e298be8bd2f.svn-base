...
<html>
	<head>
		...
		<script type="text/javascript">
			function imageManage(id){
				showDialog("图片管理","./img?id="+id,function(doc){},
						{"Width":600,"Height":500,"ShowMessageRow":false,"ShowButtonRow":false});
			}
		</script>
	</head>
	<body>	
		        ...
		        <table>
		            ...
		            <tr>
		             ...
		             <tr>
		            	...
		            	<td>制卡状态：</td>
						<td width="200"><select name="makeCardStatus">
							<option value="">--请选择--</option>
							<c:forEach items="${makeCardStatusMap}" var="item" varStatus="index">
								<option value="${item.key }"
									<c:if test="${item.key == param.makeCardStatus}">selected="selected"</c:if>>${item.value}</option>
							</c:forEach>
							</select>
						</td>
		            </tr>
		    ...
		    <div class="mainbox">
		        ...
		        <table class="datalist fixwidth">
		           <tr>
						...
						<th nowrap="nowrap" width="250">操作</th>
		            </tr>
		            ...
		            <tr>
		            	...
		            	<td  nowrap="nowrap">${item.makeCardStatusName}</td>
		            	<td  nowrap="nowrap">
		            		<ct:display model="gift_required_makeCardList" btn="view_btn">
								<a href="view/${item.batchNo}">查看</a>&nbsp; &nbsp;
							</ct:display>
							<c:if test="${item.makeCardStatus == 0}">
								<ct:display model="gift_required_makeCardList" btn="make_card_btn">
									<a href="#" onclick="dealInfo('确认制卡？','makeCard/${item.batchNo}');">制卡</a> &nbsp; &nbsp;
								</ct:display>
							</c:if>
							<c:if test="${item.makeCardStatus == 1}">
								<ct:display model="gift_required_makeCardList" btn="download_btn">
									<a href="#" onclick="dealInfo('确认下载？','getFile/${item.batchNo}');">下载文件</a> &nbsp; &nbsp;
								</ct:display>
							</c:if>
							<c:if test="${item.makeCardStatus == 1}">
								<ct:display model="gift_required_makeCardList" btn="export_btn">
									<a href="download/${item.batchNo}">导出文件</a>&nbsp; &nbsp;
								</ct:display>
							</c:if>
							<ct:display model="gift_required_makeCardList" btn="img_btn">
									<a href="#" onclick="imageManage(${item.batchNo});">管理图片</a>&nbsp; &nbsp;
							</ct:display>
						</td>
		            </tr>
		            </c:forEach>
