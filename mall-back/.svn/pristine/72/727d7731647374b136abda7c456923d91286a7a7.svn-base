   ...
   <script type="text/javascript">
    ...
    $(document).ready(function(){
    	...
    	$("#uploadPropertyPic").uploadify({
			...
			width         : 120,
			uploader:  "${ctx}/item/item/imgUpload/${itemId};jsessionid=<%=request.getRequestedSessionId()%>",
    ...
    function delImg(fileId,el){
		...
		$.ajax({
			type : "POST",
			url : "${ctx}/item/item/deleteItemImgAjax/"+fileId,
			cache : false,
			dataType : "json",
			success : function(r) {
    ...
    function coverImg(fileId){
		...
		$.ajax({
			type : "POST",
			url : "${ctx}/item/item/coverItemImgAjax/"+fileId+"/${itemId}",
			cache : false,
			dataType : "json",
			success : function(r) {
				simpleAlert(r.message,function() {
