...
//		r.region_name regionName,
		...
		sqlBuff.append(" where 1 = 1 ");
				...
				if (StringUtils.isNotEmpty(pageModuleData.getRegionCode())) {
					sqlBuff.append(" and ( ");
					String [] regionCodes = pageModuleData.getRegionCode().split(",");
					for(int i = 0;i<regionCodes.length;i++){
						if(i==0){
							sqlBuff.append(" t.region_code like ? ");
						}else{
							sqlBuff.append(" or t.region_code like ? ");
						}
						params.add("%" + regionCodes[i].trim() + "%");
					}
					sqlBuff.append(" ) ");
				}
