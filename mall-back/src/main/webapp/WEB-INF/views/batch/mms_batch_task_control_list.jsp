...
<div class="queryContainer" >
    <form name="queryForm" id="queryForm" action="mmsbatchlist" method="get">
        <table>
            <tr>
        ...
        <table class="datalist fixwidth">
            ...
            <tr>
               <td  nowrap="nowrap">${item.id}</td>
                ...
                <ct:display model="mms_manager_list" btn="view_btn">
                      <a href="viewMms/auditList/${item.id}">查看</a>
                    </ct:display>
                    &nbsp;&nbsp;
                 <c:if test="${item.status==3}">
                    <ct:display model="mms_manager_list" btn="start_btn">
                        <a href="#" onclick="dealInfo('确定恢复？','processmms/renew/${item.id}');">恢复</a>
                    </ct:display>
                    &nbsp;&nbsp;
                   </c:if> 
                    <c:if test="${item.status==4}">
                    <ct:display model="mms_manager_list" btn="pause_btn">
                        <a href="#" onclick="dealInfo('确定暂停？','processmms/pause/${item.id}');">暂停</a>
                    </ct:display>
                    &nbsp;&nbsp;
                   </c:if> 
                   <c:if test="${item.status==4}">
                    <ct:display model="mms_manager_list" btn="cancel_btn">
                        <a href="#" onclick="dealInfo('确定取消？','processmms/cancel/${item.id}');">取消</a>
                    </ct:display>
                    &nbsp;&nbsp;
                   </c:if> 
                </td>
