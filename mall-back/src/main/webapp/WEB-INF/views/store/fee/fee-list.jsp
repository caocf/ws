...
<div id="search-menu">
      <ul>
         ...
         <ct:display model="treaty_list" btn="add_btn">
          <ht:menu-btn text="添加费率" url="/store/storeFee/feeAdd?storeId=${store.id }" type="add"/>
        </ct:display>
    </ul>
    <br style="clear: left" />
</div>
...
<div class="container">
    ...
    <h3>商户费率列表</h3>
    <div class="mainbox">
        <c:if test="${not empty storeFeeList.data}">


        <table class="datalist fixwidth">
            <tr>
            	<th nowrap="nowrap">费率编号</th>
                <th nowrap="nowrap">商户名称</th>
                <th nowrap="nowrap">费率分类</th>
                <th nowrap="nowrap">清算类型</th>
                <th nowrap="nowrap">生效日期</th>
                <th nowrap="nowrap">失效日期</th>
                <th nowrap="nowrap">费用方向</th>
                <th nowrap="nowrap">收费周期</th>
                <th nowrap="nowrap">操作</th>
            </tr>

            <c:forEach items="${storeFeeList.data}" var="item">
            <tr>
            	<td  nowrap="nowrap">${item.id}</td>
                <td  nowrap="nowrap">${item.name}</td>
                <td nowrap="nowrap">${item.productionTypeName}</td>
                <td  nowrap="nowrap">${item.clearTypeName}</td>
                <td  nowrap="nowrap">
        ...
        <c:if test="${empty storeFeeList.data}">
        <div class="note">
