...
<script type="text/javascript">
	...
	$(function(){
		...
		var type = "${type}";
		if(type == 'onLine'){
			$("#backUrl").val("/giftcard/apply/list");
		}else{
			$("#backUrl").val(url);
		}
		var html= "<div><br />"+
		        "<div class='input' style='float: left;'>"+
		        	"卡批次号:"+
		            "&nbsp;&nbsp;<input name='batchNoValue' id='batchNoValue' num = '' class='small required ' readOnly onClick='selectBatchNo();'/>"+
		        	"&nbsp;&nbsp;<span class='error' id='advice-required-batchNoValue' style='display:none;color: red'></span>"+
		        "</div>"+
		        "<div class='input' style='float: left;'>"+
		            "卡数量:"+
		            "&nbsp;&nbsp;<input name='numValue' id='numValue' num = '' class='small required '/>"+
		            "&nbsp;&nbsp;<span id='numTip'></span>"+
		        	"&nbsp;&nbsp;<span class='error' id='advice-required-numValue' style='display:none;color: red'></span>"+
		        	"&nbsp;&nbsp;<span class='error' id='advice-float-range-numValue' style='display:none;color: red'></span>"+
		        "</div>"+
		    "</div><br/>";
...
<body>
	<div id="content">
		<div class="box">
			<div class="title">
				<h5>
					<c:if test="${type == 'onLine'}">线上出库申请录入</c:if>
					<c:if test="${type == 'line'}">线下出库申请录入</c:if>
				</h5>
			</div>
			<form:form method="post" id="fm" action="add" commandName="info" htmlEscape="true"
				acceptCharset="utf-8" cssClass="required-validate"
				enctype="multipart/form-data">
				<input type="hidden" id="backUrl" name="backUrl" />
				<input type="hidden" name = "orderId" value="${orderId}">
				<input type="hidden" name = "type" <c:if test="${type == 'onLine'}">value="0"</c:if><c:if test="${type == 'line'}">value="1"</c:if>>
				<div class="form">
					<div class="fields">
		                <div class="field">
