...
public class ShopUserService {
	 ...
	 * logger
	 */
	protected static final Log logger = LogFactory.getLog(ShopUserService.class);
	 ...
	 * @throws SQLException
	 */
	public Page<ShopUser> findShopUser(ShopUser user, int pageNo) throws SQLException {
		...
		if (null != user) {
			...
			if (StringUtils.isNotEmpty(user.getMobile())) {
				...
				params.add(user.getMobile().trim());
			}
			...
			/**
			 * modify by xueqiang 这个是账户ID，不是商户ID
			 */
		...
		return dbHelper.getPage(sql.toString(), ShopUser.class, pageNo, Page.DEFAULT_PAGESIZE, params.toArray());
	...
	public ShopUser saveShopUser(Long shopClass, Long shopId, Long type) {
		...
		if (type == 2L) {
			...
			try {
				...
				logger.error("创建商户异常:" + e.getMessage());
			}
		}
		return shopUser;
