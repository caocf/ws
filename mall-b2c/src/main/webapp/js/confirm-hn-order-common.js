$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (typeof data.isInSaleArea != "undefined" && !data.isInSaleArea) {
                if (!confirm("尊敬的用户，您的号码归属地与商品的销售区域不符，是否继续？")) {
                    window.location = G_CTX_ROOT + 'cart/show.chtml';
                }
            }
            buildPage(data);
            confirmOrderProcess();
    ...
    function buildPage(data) {
        ...
        for (var i = 0; i < cartItems.length; i++) {
            ...
            itemcontainer.find(':hidden[name="deliveryFee"]').val(toDecimal3(toDecimal(cartItem.deliveryFee)*ratioScore));
            ...
            var itemShopPrice = toDecimal3(toDecimal(itemInfo.item.shopPrice)*ratioScore);
    ...
    var confirmOrder = {
        ...
        _getOrderPriceAndItemIds: function () {
            var deliveryFee = 0;
            var itemIds = [];
            ...
            $('.editproduct .order_item.item_iter').each(function () {
                ...
                if (typeof cheapPrice.text() === "string" && cheapPrice.text() !== "")
                    itemPrice = cheapPrice;
                itemPrice.text(toDecimal3(toDecimal2(itemPrice.text())));
                itemTotalPrice.text(toDecimal3(toDecimal2(toDecimal(itemPrice.text()) * q)));
            ...
            shopProPrice.text("-" + toDecimal3(toDecimal2(toDecimal(shopProPrice.text())*ratioScore)));
            ...
            shopTotalPrice.text(toDecimal3(toDecimal2(total)));

            return new Array(toDecimal3(toDecimal2(total)), itemIds);
        },
        ...
        freshPrice: function () {
            $('.total_price').text(confirmOrder._getOrderPriceAndItemIds()[0]);
        },
    ...
    var confirmOrderProcess = function () {
        ...
        $('.order_item').find("[func='minus']").bind('click', {count: -1}, confirmOrder.plusPrice);
        $('.order_item').find("[func='plus']").bind('click', {count: 1}, confirmOrder.plusPrice);
        $('.order_item').find(".order_item_quantity").bind('blur', {count: 0}, confirmOrder.plusPrice);
        $('.order_item').find(".order_item_quantity").bind('change', {count: 0}, confirmOrder.plusPrice);
    ...
    $('#order_submit_btn a').click(function () {
        ...
        jsonReq('order/purchase.chtml', {
            ...
            $.removeCookie('confirm_props', {path: '/'});
