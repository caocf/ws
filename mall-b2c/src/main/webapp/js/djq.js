$(document).ready(function () {
	...
	getConsults(1,1);getPurchaseRecords(1,1);
	//遍历锚点
	var mds = $(".product_tab_cont")
	var arrMd = [];
	...
	//高亮导航菜单
	function navon(id){
	    $('.product_tab_nav li').removeClass('curr');
	    $('.product_tab_nav li').eq(id).addClass('curr');
	}
	//绑定滚动事件
...
function focus_area() {
	var areaCode = jQuery.cookie('areaCode');
	if(jQuery("#titlearea_"+areaCode).html()) {
		jQuery("#titlearea_"+areaCode).click();
	}else if(jQuery(".titlearea").get(0)){
		jQuery(".titlearea").get(0).click();
	}else{
...
function getBasicInfo() {
    $.get(webRoot + "item/basicInfo.chtml?saleId=" + itemId + "&ramdom=" + Math.random(), function (data) {
            if (data != null) {
                ...
                $("#choose-btns").html(btns);

                getLimitTime(data.promotionEndTime);
                //限时抢购商品
//                if (data.iseckill === '3') {
//
//                    $("#spec-n1").after("<div class=\"goods-discount-tag\"></div>");
//                    $("#summary-price .col_link:first").addClass("old_price");
//                    $("#memPrice").after("<li><div class=\"dt long-dt\">距特价结束还有：</div>"
              ...
              //限时购商品
				...
				if(data.iseckill==='5'){
					if(data.othersItems==null){
                        ...
                        $("#product_menu .wrap-input,#product_menu .btn").hide();
                   		} else {
                        $("#product_menu").addClass("nav-fixed");
						$("#product_menu .wrap-input,#product_menu .btn").show();
						...
						var $product_menu_top = $("#product_menu").offset().top;
						//$(".djq_box_more > a").attr('href',data.show_url);	
               		 	$(window).scroll(function () {
...
function getDistountItems() {
    $.get(webRoot + "item/distountItems.chtml?saleId=" + itemId + "&ramdom=" + Math.random(), function (data) {
            ...
            $(".product_tab").each(function () {
                $(this).children(".product_tab_wrap").children(".product_tab_cont").show();
            });
...
function numcheck() {
	if(isNaN($("#buy-num").val()) || isNaN($("#buy-num2").val())) {
		$("#buy-num").val("1");
		$("#buy-num2").val("1");
		return;
	}
	var wantBuy = parseInt($("#buy-num").val());
    var wantBuy2 = parseInt($("#buy-num2").val());
    var kc = parseInt(kucun);
    if ((wantBuy > 200 || wantBuy2>200)) {
    	if(kc>200) {
    		$("#buy-num").val("200");
            $("#buy-num2").val("200");
    	}else if(kc>0){
    ...
    if (wantBuy < 1 || wantBuy2<1) {
        $("#buy-num").val("1");
        $("#buy-num2").val("1");
        //$("#kucunTips").html("<div class='clearfix new_goutip'><div class='inew_goutip'><span></span>您所填写的数量不能小于1!</div></div>");
    } else if (kc != "-1000" && (kc < wantBuy|| kc < wantBuy2)) {
        $("#buy-num").val(kc);
        $("#buy-num2").val(kc);
        //$("#kucunTips").html("<div class='clearfix new_goutip'><div class='inew_goutip'><span></span>您所填写的数量超出商品库存的数量!</div></div>");
    } else {
...
function addCart() {
    ...
    if(popBindMobile($('#InitCartUrl').offset().top,itemId,'djq')){return;}
    var info = {
        "itemId": itemId,
