        ...
        $.get(webRoot + "item/historyItem.chtml?saleIds=" + historys + "&ramdom=" + Math.random(), function (data) {
                ...
                for (var j = 0; j < hises.length; j++) {
                    for (var i = 0; i < k.length; i++) {
                        var history = k[i].item;
                        if (hises[j] == history.id) {
                            var name = history.shortName;
                            if (name == '') {
                                name = history.name;
                            }
                            if(history.iseckill==5){
                            	html += '<li><div class="p-img" style="margin-top: 3px;">';
                            	html += '<a href="' + history.itemPath + '"><img data-img="1" width="50" height="30" src="' + history.imgPath + 'N5/' + history.webPath + '" class="err-product"></a></div>';
                            	html += '<div>';
                            }else{
                            	html += '<li><div class="p-img">';
                            	html += '<a href="' + history.itemPath + '"><img data-img="1" width="50" height="50" src="' + history.imgPath + 'N5/' + history.webPath + '" class="err-product"></a></div>';
                            	html += '<div class="p-name">';
                            }
                            html +='<a title="' + history.name + '" href="' + history.itemPath + '">' + name + '</a></div>';
							if(history.source != '5'){
                            	if(history.source == '4'){
                            		html += '<div class="p-price">积分' + FloatMul(history.shopPrice, 100) + '</div></li>';
                            	}else{
                            		html += '<div class="p-price">￥' + toDecimal2(history.shopPrice) + '</div></li>';
                            	}
                            }
                        }
                    }
...
function getRecommendQuan(){
	$.ajax({
        url:webRoot + 'recommend/xqdj.chtml',
        dataType: "json",
        cache:false,
        success:function(data){
        	if($.isArray(data) && data.length > 0){
    			var tujian = "<div class='related-content'><ul id='related_his'>";
    			for (var i = 0; i < data.length; i++){
    				tujian += "<li><div class='p-img'><a href='http://quan.12580life.com/product/i"+ data[i].ID +".htm?referer="+data[i].REFERER+"'>";
    				tujian += "<img class='err-product' width='50' height='50' data-img='1' src='http://quan.12580life.com/shlm/upload/o2o" + data[i].GOODS_PIC_PATH + "'></a></div>";
    				tujian += "<div class='p-name'><a title='"+data[i].GOOD_TITLE+"' href='http://quan.12580life.com/product/i"+data[i].ID+".htm?referer="+data[i].REFERER+"'>"+data[i].GOOD_TITLE+"</a></div>";
    				tujian += "<div class='p-price'>￥" + data[i].SALES_PRICE + "</div></li>";
    			}
    			tujian += "</ul></div>";
    			$(tujian).insertAfter("#sidebar_recomment .related-title");
    		}else{
    			$("#sidebar_recomment").hide();
    		}
        }
     });
}
            ...
            if (data != null) {
                ...
                if (data.itemMode === '0') {
                    if (data.postFlag === '0') {
                        ...
                        if (data.logisticsFee === '0.00' || data.logisticsFee === '0') {
                            $("#feeNum").html('包邮');
                        } else {
                            $("#feeNum").html('￥' + data.logisticsFee+'起');
                        }
                    }
                } else {
                    $("#feeNum").html('无需物流');
                    ...
                    //虚拟商品判断地图是否显示
                    if(data.verify_shop == 'false'){
                 	   $('#map_canvas').parent().parent().parent().hide();
                    }
                }
                imgPath = data.imgPath;
                $("#memPrice").html(data.memPrice);

                if (data.userLevels === 'L1') {
                    $("#product_info #name h1").addClass("hongzuan");
                }
                ...
                if (data.isValid !== "1") {
                    ...
                    if (data.itemMode === '0') {
                        btns = "<div class=\"btn\" id=\"choose-btn-append\"> <a href=\"javascript:void(0);\" onclick=\"addCart();return false;\" id=\"InitCartUrl\" class=\"btn-append \" title=\"\">加入购物车<b></b></a> </div>";
                    }
                    btns += " <div class=\"btn\" id=\"choose-btn-easybuy\" style=\"display: block;\"><a href=\"javascript:void(0);\" id=\"InitCartUrl\" onclick=\"buyNow();return false;\" class=\"btn-easybuy \" title=\"\">立即购买<b></b></a></div>"
                ...
                if (data.iseckill=='9'){
                	 getLimitTime(data.promotionStartTime, 'START');
                }else{
                	 getLimitTime(data.promotionEndTime, 'END');
                }

                //限时购商品
				if(data.iseckill==='3'){
					var html="<li id=\"summary-time\">"
						+"<div>活动售完为止，活动时间 <span class=\"red\">"
						+ data.startTime+"</span> 至 <span class=\"red\">"+ data.stopTime+"</span></div>"
						+"</li>";
					$("#summary-tips").before(html);
					$("#spec-n1").after("<div class=\"icon_xsg\"><img  src=\"../img/icon_xsg.png\" /></div>");
					if(data.isSaleValid==='false'){
						$("#choose-btns").remove();
					}
				}else if (data.iseckill=='9'){
					var stocknum = data.stocknum;
					stocknum = stocknum.replace("库存","");
					var appendObj = $('#choose-btn-append').find('a');
					var easybuyObj = $('#choose-btn-easybuy').find('a');
					if (stocknum > 0){
						$("#spec-n1").before("<div class=\"goods-discount-tag\"></div>");
						if(appendObj){
							appendObj.attr('onclick', 'addCart();return false;');
							appendObj.removeClass('btn-append');
							appendObj.addClass('btn-append');
						}
						if(easybuyObj){
							easybuyObj.attr('onclick', 'buyNow();return false;');
							easybuyObj.removeClass('btn-un-easybuy');
							easybuyObj.addClass('btn-easybuy');
						}
					}else{
						$("#spec-n1").before("<div class=\"goods-detail-soldout-tag-show\"></div>");
						// 显示售罄图片
						if(appendObj){
							appendObj.removeAttr('onclick');
							appendObj.removeClass('btn-append');
							appendObj.addClass('btn-un-append');
						}
						if(easybuyObj){
							easybuyObj.removeAttr('onclick');
							easybuyObj.removeClass('btn-easybuy');
							easybuyObj.addClass('btn-un-easybuy');
						}
					}
                    limitEndTime = data.promotionEndTime;
                    $("#memPrice").after("<li><div class=\"dt long-dt timeLimit\">距特价开始还有：</div>"
                       + "<div class=\"dd long-dd\">"
                       + "<div id=\"mo-date\">"
                       + "<span id=\"mo-day\">00</span>&nbsp;天&nbsp;<span id=\"mo-hour\">00</span>&nbsp;时&nbsp;<span id=\"mo-min\">00</span>&nbsp;分"
                       + " </div>");
                    if(data.userLevels){
                    	 var html = "";
                         if (data.isRedMember === 'L1') {
                             html = "<span class=\"col999\">&lt;你已经是红钻会员&gt;</span>";
                         } else {
                             html = " <a class=\"for_vip\" href=\"../static/vip/index.htm\">成为红钻会员</a>";

                         }
                         $('#summary-price strong.p-price').after(html);
                    }
                    $('#buy-num').attr("max", data.userPerBuyNum);
                    if(data.userPerBuyNum && parseInt(data.userPerBuyNum) > 0){
                    	$("#kucunTips").before("<span>（每人限购"+data.userPerBuyNum+"件）</span>");
                    }
				}else{
                    var html = "";
                    if (data.isRedMember === 'L1') {
                    ...
                    $("#memPrice #red_price .dd strong").after(html);
                }
				
				if(data.iseckill==='5'){
					if(data.othersItems==null){
						$(".other_djq").hide();
					}else{
						$(".other_djq").show();
						$(".djq_box_cont").html(data.othersItems);
					}
				...
				var valueMethod = $(".pay_mth").html();
				if(null!=valueMethod && ""!=valueMethod){
					if(valueMethod.length>5){
						$(".pay_mth").append("<b style='color:#999999;font-weight: normal;'>(积分、话费、商城币支付仅限江苏移动用户)</b>");
					}
				}
...
function getPurchaseRecords() {
    $.get(webRoot + "item/purchaseRecords.chtml?saleId=" + itemId + "&curpage=" + arguments[0] + "&ramdom=" + Math.random(), function (data) {
            ...
            if(undefined != arguments[1]) {
            	recordsHtml = "<H2>成交记录</H2>"+recordsHtml;
            }
    ...
    $.get(webRoot + "item/distountItems.chtml?saleId=" + itemId + "&ramdom=" + Math.random(), function (data) {
            ...
            if ($("#rr").html != '') {//功能条跟随
                ...
                $(window).scroll(function () {
                    ...
                    if ($product_menu_top - $(document).scrollTop() > -30) {
            ...
            $("#tab_detail .tab li a").click(function () {
                ...
                $(this).parents(".#tab_detail").find(".product_tab_cont").hide().filter($(this).attr("href")).show(); return false;
            });
            //换一套推荐产品
...
function upNum() {
	$("#kucunTips").siblings('.prod-limit-time').remove();
    var wantBuy = $("#buy-num").val() - (-1);
    if (wantBuy > 200) {
        $("#kucunTips").after('<div class="prod-limit-time" style="color:#FF0000;">您所填写的数量超出最大购买限制</div>');
        return;
    }else if($("#buy-num").attr('max') && $("#buy-num").attr('max') != 0 && (wantBuy - $("#buy-num").attr('max') > 0)){
    	$("#kucunTips").after('<div class="prod-limit-time" style="color:#FF0000;">限时特价商品，每人限购'+$("#buy-num").attr('max')+'件</div>');
    	return;
    }
    if (kucun != "-1000" && kucun < wantBuy) {
        $("#kucunTips").after('<div class="prod-limit-time" style="color:#FF0000;">您所填写的数量超出商品库存的数量!</div>');
    } else {
        $("#buy-num").val(wantBuy);
        $("#kucunTips").html("");
    }
...
function downNum() {
    ...
    var wantBuy = $("#buy-num").val() - 1;
    if (wantBuy < 1) {
        $("#kucunTips").after('<div class="prod-limit-time" style="color:#FF0000;">您所填写的数量不能小于1!</div>');
    } else {
        $("#buy-num").val(wantBuy);
        $("#kucunTips").html("");
    }
...
function numcheck() {
	$("#kucunTips").siblings('.prod-limit-time').remove();
	if(isNaN($("#buy-num").val())) {
		$("#buy-num").val("1");
		return;
	}
    var wantBuy = parseInt($("#buy-num").val());
    var kc = parseInt(kucun);
    if (wantBuy > 200) {
    	if(kc>200) {
    		$("#buy-num").val("200");
    	}else if(kc>0){
    		if($("#buy-num").attr('max') && $("#buy-num").attr('max') != 0 && parseInt($("#buy-num").attr('max')) > 0){
    			if(parseInt($("#buy-num").attr('max')) - kc < 0){
    				$("#buy-num").val(parseInt($("#buy-num").attr('max')));
    			}else{
    				$("#buy-num").val(kc);
    			}
    		}else{
...
function addCart() {
    ...
    if(popBindMobile($('#InitCartUrl').offset().top,itemId,'sc')){return;}
    var info = {
        "itemId": itemId,
...
//弹出绑定手机号界面
var bindMobile_regionCode = '320000';
var bindMobile_areaYd = '';
var bindMobile_tipInfo = '尊敬的客户，您所购买的商品，需要绑定<strong style="color:#2777c6; font-size:14px;">移动</strong>的手机号码后，才能继续进行。';
$('html>head').append('<script type="text/javascript" src="http://mall.12580life.com/js/bind_mobile.js"></script>');
function buyNow() {
	//插码：“立即购买”事件码
	if(typeof(_tag)!='undefined'){
    ...
    if(popBindMobile($('#InitCartUrl').offset().top,itemId1,'sc')){return;}
    $.ajax({
...
function getLimitTime(limitTime, flag) {
    ...
    function updateTime() {
        ...
        var iRemain = 0;
        if(flag == 'START'){
        	iRemain = (oDateNow.getTime() - oDateEnd.getTime()) / 1000;
        	if (iRemain >= 0) {
        		$('div.timeLimit').text('距特价结束还有：');
        		if(!isFirst){
        			 clearInterval(timer);
                     clearInterval(offsetTimer);
                     iRemain = 0;
        			 getLimitTime(limitEndTime, 'END');
                     isFirst = true;
        		}
        	}
        }else{
