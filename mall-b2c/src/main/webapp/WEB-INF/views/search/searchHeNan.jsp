...
<META HTTP-EQUIV="pragma" CONTENT="no-cache">
	<META HTTP-EQUIV="Cache-Control" CONTENT="no-cache, must-revalidate">
		...
		<script type="text/javascript" src="../js/searchHeNan.js"></script>
	 ...
	 $(document).ready(function(){
			var str = "<a>全部结果</a>";
  			...
  			if(menuTab.flag=="ok"){
  				for(var i=1;i<menuTab.menu.length;i++){
		...
		<div id="search_container" class="w750 bt10 fr">
			<div id="select">
				...
				<input type="hidden" id="store_id" name="store_id"
					value="${store_id }" /> <input type="hidden" id="type_id"
					name="type_id" value="${type_id }" />
					<input type="hidden" id="region_code" name="region_code" value="${region_code }"/> <input type="hidden"
					id="brand" name="brand" value="${brand }" /> <input type="hidden"
					id="sort" name="sort" value="${sort }" /> <input type="hidden"
					id="curpage" name="curpage" value="${curpage }" />
				<input type="hidden" id="searchSource" name="searchSource" value="${searchSource }" />
				<!-- 是否点击了条件选择 -->
				<input type="hidden" id="myparams" name="myparams"
					value="${myparams }" /> <input type="hidden" id="hideType"
								...
								<c:if test="${json.DATA!=null }">
									<c:forEach items="${json.DATA}" var="item">
										<div class="items">
											...
											<div class="price">
												<c:if test="${item.G_SOURCE=='4' }">
												<span class="old_price">${item.G_MARKET_PRICE }</span> <span
													class="now_price">积分<strong>
													 <fmt:formatNumber type="number" maxFractionDigits="0">${item.G_SHOP_PRICE*100}</fmt:formatNumber>
													</strong>
												</span>
												</c:if>
											</div>
