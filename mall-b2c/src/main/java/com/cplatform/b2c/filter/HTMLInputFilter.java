...
public class HTMLInputFilter {
	...
	/** ���� */
	private static HTMLInputFilter instance = null;

	public synchronized static HTMLInputFilter getInstance() {
		if (instance == null) {
			instance = new HTMLInputFilter(false);
		}
		return instance;
	}

	private HTMLInputFilter(boolean debug) {
		vDebug = debug;
	...
	public static boolean ifContainXss(String value) {
		String valusa = HTMLInputFilter.getInstance().filter(value);
		if (value.equals(valusa)) {
			return false;
		} else {
			return true;
		}
